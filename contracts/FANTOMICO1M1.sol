...
contract FantomICO1M1 is Ownable, ReentrancyGuard {
    ...
    uint256 public depositRate = 100; // 1 usd = 100 * baby
...
// UDSC: 0x04068DA6C83AFCFA0e13ba15A6696662335D5B75 

    constructor(IERC20 _dai, IERC20 _usdc, IERC20 _baby, address _masterWallet) {
        require(address(_dai) != address(0), "dai address should not be zero address");
    ...
    function Depositusd(uint256 _amount, uint256 _type) external nonReentrant {
        require(_amount > 0, "USD Amount is less than zero");
        ...
        require(_type >= 0 && _type <= 1, "Type should be in range of 0 to 1");

        uint256 babyTokenAmount;
        uint256 _tokenDecimals;

        if (_type == 0) {
            _tokenDecimals = dai.decimals();
            babyTokenAmount = _amount.mul(depositRate).mul(10 ** baby.decimals()).div(10 ** _tokenDecimals);
            require((baby.balanceOf(msg.sender) + babyTokenAmount) <= _hardCap * 10 ** 18, "Cannot hold more than 100000 $BABY in your wallet");
            dai.safeTransferFrom(msg.sender, masterWallet, _amount);
        }
        if(_type == 1){
    ...
    function updatebabyToken(IERC20 _baby) external onlyOwner{
        require(address(_baby) != address(0), "BABY Token address should not be zero address");
        baby = _baby;
    }
    ...
    function setEndTime(uint256 _etime) external onlyOwner {
        require(_etime > startTime, "Cannot be zero");
        endTime = _etime;
    }
